...
function ElizaBot(script) {
	this.scr = script;    
        this.noRandom= true; //(noRandomFlag)? true:false;
...
ElizaBot.prototype._init = function() {
	...
	var synPatterns={};
	if ((this.scr.synon) && (typeof this.scr.synon == 'object')) {
	    for (var i in this.scr.synon) {
		synPatterns[i]='('+i+'|'+this.scr.synon[i].join('|')+')';
	    }
	}
	// check for keywords or install empty structure to prevent any errors
	if ((!this.scr.key) || (typeof this.scr.key.length == 'undefined')) {
		this.scr.key=[['###',0,[['###',[]]]]];
	}
	...
	for (var k=0; k<this.scr.key.length; k++) {
		var rules=this.scr.key[k][2];
		this.scr.key[k][3]=k; // save original index for sorting
	...
	this.scr.key.sort(this._sortKeywords);
	// and compose regexps and refs for pres and posts
	ElizaBot.prototype.pres={};
	ElizaBot.prototype.posts={};
	if ((this.scr.pre) && (this.scr.pre.length)) {
		var a=new Array();
		for (var i=0; i<this.scr.pre.length; i+=2) {
			a.push(this.scr.pre[i]);
			ElizaBot.prototype.pres[this.scr.pre[i]]=this.scr.pre[i+1];
		}
	...
	if ((this.scr.post) && (this.scr.post.length)) {
		var a=new Array();
		for (var i=0; i<this.scr.post.length; i+=2) {
			a.push(this.scr.post[i]);
			ElizaBot.prototype.posts[this.scr.post[i]]=this.scr.post[i+1];
		}
	...
	// check for quitmsg and install default if missing
	if ((!this.scr.quitmsg) || (typeof this.scr.quitmsg.length == 'undefined')) {
		this.scr.quitmsg=[];
	}
	// done
...
ElizaBot.prototype.transform = function(text) {
	...
	for (var i=0; i<parts.length; i++) {
		...
		if (part!='') {
			...
			for (var q=0; q<this.scr.quitmsg.length; q++) {
				if (this.scr.quitmsg[q]==part) {
					this.quit=true;
			...
			for (var k=0; k<this.scr.key.length; k++) {
				if (part.search(new RegExp('\\b'+this.scr.key[k][0]+'\\b', 'i'))>=0) {
...
ElizaBot.prototype._execRule = function(k) {
	...
	for (var i=0; i<decomps.length; i++) {
		...
		if (m!=null) {
			...
			var rpl=reasmbs[ri];
			if (this.debug) alert('match:\nkey: '+this.scr.key[k][0]+
				'\nrank: '+this.scr.key[k][1]+
				'\ndecomp: '+decomps[i][0]+
				'\nreasmb: '+rpl+
				'\nmemflag: '+memflag);
...
ElizaBot.prototype._getRuleIndexByKey = function(key) {
	for (var k=0; k<this.scr.key.length; k++) {
		if (this.scr.key[k][0]==key) return k;
	}
	return -1;
}
...
ElizaBot.prototype.getFinal = function() {
    if (!this.scr.lastmsg) return '';
    return this.scr.lastmsg[Math.floor(Math.random()*this.scr.lastmsg.length)];
}

ElizaBot.prototype.getInitial = function() {
    if (!this.scr.initmsg) return '';
    return this.scr.initmsg[Math.floor(Math.random()*this.scr.initmsg.length)];
}
...
ElizaBot.prototype.print = function() {
    this.scr.initmsg.forEach(function(msg, i) {
	console.log("init["+i+"]: \""+msg+"\"");
    });
    this.scr.lastmsg.forEach(function(msg, i) {
	console.log("last["+i+"]: \""+msg+"\"");
    });
    this.scr.quitmsg.forEach(function(msg, i) {
	console.log("quit["+i+"]: \""+msg+"\"");
    });
    this.scr.pre.forEach(function(a, i) {
	console.log("pre["+i+"]: \""+a[0]+"\" \""+a[1]+"\"");
    });
    this.scr.post.forEach(function(a, i) {
	console.log("post["+i+"]: \""+a[0]+"\" \""+a[1]+"\"");
    });

    var keys = Object.keys(this.scr.synon);
    var vals = Object.values(this.scr.synon);
    keys.forEach(function(a, i) {
	console.log("synon["+a+"]: "+vals[i]);
    });

    this.scr.key.forEach(function(a, i) {
	console.log("key["+i+"]: word: \""+a[0]+"\" rank: "+a[1]);
        a[2].forEach(function(b, j) {
	    console.log("    decomp: \""+b[0]+"\"");
	    b[1].forEach(function(c, k) {
		console.log("        reasmbl["+k+"]: \""+c+"\"");
	    });
        });
    });
}
